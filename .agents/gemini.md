...
- **Local Binary Execution Constraints**: 
  *Active Constraint*: Read [`.agents/skills/edr-constraints/SKILL.md`](skills/edr-constraints/SKILL.md) before running any Go test or local binary -- it is the single source of truth for this topic (per `AGENTS.md`'s own "one topic, one file" rule). Do not restate its rule here; it changes independently of this file.
...
- [x] Fetch Personal Access Tokens from `/api/tokens` and implement 'Export CSV' button in V2 `Dashboard.tsx` (#708).

## V2 Portal Pending Registrations Sidebar Badge (v1.34.5)
- [x] Implement pending registrations count badge next to 'Users' link in V2 `Sidebar.tsx` (#709).

## V2 Portal Telemetry Dashboard Port (v1.34.6)
...
## Graceful Edge Node Shutdown Warnings & Client Failover Hooks (Epic #1083)
- [x] Sub-Issue #1084: Server-side Edge Shutdown Warning Scheduler & Config
- [x] Sub-Issue #1085: Client WebSocket Shutdown Notification Handling & Auto-Failover
- [x] Sub-Issue #1086: Client Lifecycle Hook Execution Engine
- [x] Sub-Issue #1087: Integration Tests & Documentation for Client Failover Hooks
