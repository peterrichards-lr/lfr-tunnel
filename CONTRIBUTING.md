...
5. **Code Health, DRY, and Complexity Guidelines**:
   To maintain a clean and sustainable architecture, follow these guidelines:
   - **Keep Functions Concise**: Limit functions to under 100 lines (50 statements) where possible.
   - **Adhere to DRY**: Do not duplicate process checking (`IsPIDRunning`), permissions check (`checkInsecurePermissions`), or configuration resolution. Reusable code must reside in shared packages (e.g., `pkg/client` or `pkg/config`).
   - **Single Source of Truth**: Dynamic values, such as the current package version, must refer to centralized configurations (e.g., `config.Version`).
6. **Update Documentation**: If you change or add a command, or modify existing logic, please update the relevant files in the `docs/` folder. This requirement applies to all changes, including those assisted or generated by AI.

   **A new page under `docs/` also needs a `nav:` entry in `mkdocs.yml`.** The navigation is hand-maintained — `plugins:` is only `search`, so nothing derives it from the tree — and a page left out of it still builds and deploys, reachable only by direct URL or site search. `mkdocs --strict` reports that at INFO, so nothing fails. Run `make check-docs-nav` before pushing; CI's Documentation Build runs it too. A page that genuinely does not belong on the site is excluded explicitly and with a reason, as a `# nav-exclude:` line in `mkdocs.yml` — `docs/docker_hub_readme.md` is the only such page today (#1918).

   There is no `CHANGELOG.md` to update, and this clause used to ask for one (#1811). The release notes users actually read are `pkg/server/static/whats-new.json`, which `scripts/create-release-tag.sh` populates automatically from the commits in the release — so it is generated, versioned and served to the portal without anyone maintaining it by hand. A second, hand-written changelog would be a copy of it that drifts.
7. **PR Testing Artifacts**: Pull Requests automatically compile standalone binaries for Linux, macOS, and Windows. Reviewers and QA testers can download and run these binaries directly from the PR's Actions run summary page to verify changes without needing to set up a local development environment.
