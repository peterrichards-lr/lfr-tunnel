...
test-hooks:
	...
	@./tests/hooks/test-pre-push-range.sh
	@./tests/hooks/test-gate-scope-boundaries.sh
...
# A link printed by the binary is documentation too. pkg/config/metadata.go held an anchor no
# heading has ever matched, and the gateway serves it to the portal as a live href (#2081).
check-doc-links:
	@python3 scripts/check-doc-links.py

# A page no landing page names is reachable only by site search (#2081).
check-docs-routing:
	@python3 scripts/check-docs-routing.py
...
# V1 and V2 are an A/B test: the same person may be served either, so a capability in one arm is
# not a capability. Access control lived only in V2 and nobody noticed, because each arm was only
...
check-access-mode-parity:
	@node scripts/check-access-mode-parity.cjs
