...
func (repo *SQLiteUserRepo) CreateUser(u *User) error {
	...
	_, err := repo.conn.Exec(`
		INSERT INTO users (id, email, first_name, last_name, preferred_name, role, status, verification_token, approval_token, claim_token, timezone, auth_method, theme_preference, notification_prefs, created_at, updated_at, last_client_version, last_client_os, totp_secret, totp_enabled, policy_consent_at, diagnostics_consent_at, language_preference, rate_limit, max_reservations, max_custom_domains, max_active_tunnels, bandwidth_quota_bytes, onboarding_status, onboarding_last_step, onboarding_reruns, preferred_domain, subdomain_style)
		VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
	`, u.ID, u.Email, u.FirstName, u.LastName, u.PreferredName, u.Role, u.Status, u.VerificationToken, u.ApprovalToken, u.ClaimToken, u.Timezone, u.AuthMethod, u.ThemePreference, u.NotificationPrefs, u.CreatedAt, u.UpdatedAt, u.LastClientVersion, u.LastClientOS, u.TOTPSecret, totpEnabledVal, u.PolicyConsentAt, u.DiagnosticsConsentAt, u.LanguagePreference, u.RateLimit, u.MaxReservations, u.MaxCustomDomains, u.MaxTunnels, u.BandwidthQuotaBytes, u.OnboardingStatus, u.OnboardingLastStep, u.OnboardingReruns, u.PreferredDomain, u.SubdomainStyle)
	return err
}
...
func (repo *SQLiteUserRepo) fetchUserByQuery(query string, arg interface{}) (*User, error) {
	...
	if maxCustomDomainsVal.Valid {
		val := int(maxCustomDomainsVal.Int64)
		u.MaxCustomDomains = &val
	} else {
		u.MaxCustomDomains = nil
	}
	...
	if bandwidthQuotaVal.Valid {
		quota := bandwidthQuotaVal.Int64
		u.BandwidthQuotaBytes = &quota
	} else {
		u.BandwidthQuotaBytes = nil
	}
	if policyConsentAt.Valid {
		u.PolicyConsentAt = &policyConsentAt.Time
	}
	if diagnosticsConsentAt.Valid {
		u.DiagnosticsConsentAt = &diagnosticsConsentAt.Time
	}
	if lastLogin.Valid {
...
func (repo *SQLiteUserRepo) ListUsers() ([]*User, error) {
	...
	for rows.Next() {
		...
		var prefDomVal sql.NullString
		var subStyleVal sql.NullString
		if err := rows.Scan(&u.ID, &u.Email, &u.FirstName, &u.LastName, &u.PreferredName, &u.Role, &u.Status, &vt, &at, &ct, &u.Timezone, &u.AuthMethod, &u.ThemePreference, &u.NotificationPrefs, &u.CreatedAt, &u.UpdatedAt, &lastLogin, &u.LastLoginIP, &lastClientVersion, &lastClientOS, &totpSecret, &totpEnabled, &policyConsentAt, &diagnosticsConsentAt, &langPref, &rateLimitVal, &maxReservationsVal, &maxCustomDomainsVal, &maxActiveTunnelsVal, &bandwidthQuotaVal, &u.OnboardingStatus, &u.OnboardingLastStep, &u.OnboardingReruns, &prefDomVal, &subStyleVal); err != nil {
			return nil, err
		}
		u.VerificationToken = vt.String
		u.LastClientVersion = lastClientVersion.String
