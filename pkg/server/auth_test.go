...
func TestRegistryRegister(t *testing.T) {
	...
	// Register
	token, remotes, err := reg.Register("test-user", "alpha-se", ports, domains, 0, "127.0.0.1", "", nil)
	if err != nil {
...
func TestRegistryCleanup(t *testing.T) {
	...
	token, _, err := reg.Register("test-user", "gamma-se", []PortMapping{{LocalPort: 8080}}, domains, 0, "127.0.0.1", "", nil)
	if err != nil {
...
func TestRegistryValidation(t *testing.T) {
	...
	for _, tt := range tests {
		_, _, err := reg.Register("test-user", tt.subdomain, ports, domains, 0, "127.0.0.1", "", nil)
...
func TestRegistryCheckSubdomain(t *testing.T) {
	...
	_, _, err := reg.Register("test-user", "alpha-se", []PortMapping{{LocalPort: 8080}}, domains, 0, "127.0.0.1", "", nil)
	if err != nil {
		t.Fatalf("registration failed: %v", err)
	}
