...
function tableSubject(tbodyId) {
  return String(tbodyId)
    .replace(/-table-body$/, '')
    .replace(/-/g, ' ')
    .trim();
}
...
function searchLabelFor(tbodyId) {
  const subject = tableSubject(tbodyId);
  if (!subject) return t('search', 'Search');
  return `${t('search', 'Search')} ${subject}`;
}

// The page selector needs naming for the same reason the search box does, and needs to differ
// per table for the same reason: a page carrying several tables would otherwise offer several
// controls all called "Page".
//
// This one only renders when a table has more than one page, which is why it survived the first
// pass -- a fresh local database had too little data to paginate, while CI accumulates state
// across the suite and paginated the audit and magic-link tables (#1584).
function pageLabelFor(tbodyId) {
  const subject = tableSubject(tbodyId);
  if (!subject) return t('page', 'Page');
  return `${t('page', 'Page')}, ${subject}`;
}
    ...
    headers.forEach((th, index) => {
      ...
      if (sortKey) {
        ...
        th.addEventListener('click', () => {
          ...
          headers.forEach(
            (h) => (h.innerText = h.innerText.replace(/ [↑↓]$/, '')),
          );
...
function updateTableView(tbodyId) {
  ...
  if (totalPages > 1) {
    ...
    pageSelect.onchange = (e) => {
      inst.currentPage = parseInt(e.target.value);
      updateTableView(tbodyId);
    };
    ...
    if (vRes.ok) {
      ...
      if (accPrefSelect) {
        if (rule === 'user-preference') {
          ...
          if (accPrefHelp)
            accPrefHelp.textContent =
              'Preferred Domain is active because the administrator has configured User Preference domain allocation.';
        } else {
          accPrefSelect.setAttribute('disabled', 'true');
          if (accPrefHelp)
            accPrefHelp.textContent =
              'Preferred Domain selection is disabled because the administrator has configured ' +
              rule +
              ' domain allocation.';
        }
      }
      ...
      if (displayVer) {
        const versionDisplays = document.querySelectorAll(
      ...
      if (vData.latest_version) {
        const clientDisplays = document.querySelectorAll(
  ...
  if (magicToken) {
    ...
    if (vRes.ok) {
      ...
      if (data.status === 'mfa_required') {
        ...
        if (document.getElementById('sso-container'))
          document.getElementById('sso-container').style.display = 'none';
        if (document.getElementById('sso-divider'))
...
async function showDashboard() {
  ...
  if (currentUser.killed_previous_session) {
    setTimeout(
      () =>
        showToast(
          'Warning: You were previously logged in elsewhere. That session has been invalidated.',
        ),
      1000,
    );
  }
  ...
  let welcomeGreeting = greetingName
    ? `Welcome Back, ${escapeHTML(greetingName)}!`
    : 'Welcome Back!';
  let firstGreeting = greetingName
    ? `Welcome to Liferay Tunnel, ${escapeHTML(greetingName)}!`
    : 'Welcome to Liferay Tunnel!';
  if (
    currentUser.last_login_at &&
    !currentUser.last_login_at.startsWith('0001')
  ) {
    document.getElementById('last-login-banner').style.display = 'flex';
    document.getElementById('last-login-text').innerHTML =
      `<strong>${welcomeGreeting}</strong> Your last login was ${renderTimestamp(currentUser.last_login_at)} from IP <code>${escapeHTML(currentUser.last_login_ip || 'Unknown')}</code>.`;
  } else {
    document.getElementById('last-login-banner').style.display = 'flex';
    document.getElementById('last-login-text').innerHTML =
      `<strong>${firstGreeting}</strong> We're glad you're here. This appears to be your first time logging in.`;
  }
  ...
  try {
    ...
    if (vRes.ok) {
      ...
      const repoUrl =
        vData.repository_url ||
        'https://github.com/peterrichards-lr/lfr-tunnel';
      ...
      const latestVer = rawLatest.startsWith('v')
        ? rawLatest
        : rawLatest
          ? 'v' + rawLatest
          : '';
      const userVer = rawUser.startsWith('v')
        ? rawUser
        : rawUser
          ? 'v' + rawUser
          : '';
      ...
      const rawUrl =
        repoUrl.replace('github.com', 'raw.githubusercontent.com') + '/master';
      const checksumsUrl =
      ...
      let dlUrl =
        config && config.url
          ? config.url
          : `${window.location.origin}/static/downloads/${binaryName}`;
      ...
      if (
        config &&
        config.show_download !== undefined &&
        config.show_download !== null
      ) {
      ...
      let downloadLabel =
        config && config.download_label
          ? config.download_label
          : '⬇️ Download Binary';

      // Custom labels
      let cmdLabel =
        config && config.cmd_label
          ? config.cmd_label
          : '🚀 Recommended Installation (EDR Whitelisted):';
      let cmdFallbackLabel =
        config && config.cmd_fallback_label
          ? config.cmd_fallback_label
          : '🛠️ Direct Script Fallback:';
      ...
      if (config && config.recommended) {
        ...
        if (config.recommended === 'cmd_fallback') {
          if (!config.cmd_fallback_label)
            cmdFallbackLabel = '🚀 Recommended (Direct Script):';
  ...
  if (currentUser.role === 'admin' || currentUser.role === 'owner') {
    ...
    updateMaintenanceModeUI(
      currentUser.maintenance_mode || 'false',
      currentUser.iron_curtain || false,
    );
  }

  // Populate account fields
  document.getElementById('acc-first-name').value =
  ...
  setupDropdownA11y(
    'acc-custom-dropdown-trigger',
    'acc-custom-menu',
    selectAccLanguage,
    () => document.getElementById('acc-language').value,
  );
  document.getElementById('acc-notifications').checked =
  ...
  if (
    window.latestVersionData &&
    window.latestVersionData.enable_onboarding &&
    currentUser.onboarding_status === 'pending'
  ) {
...
function handleTelemetryPayload(data) {
  ...
  if (data.maintenance_mode === 'pending') {
    ...
    if (
      ...
      showToast(
        'The portal has entered scheduled maintenance. Standard sessions are suspended.',
        'danger',
      );
      setTimeout(() => {
        logout();
...
async function setBroadcastMessage() {
  ...
  if (
    !confirm(
      t('confirm_broadcast', 'Send this message to every user right now?') +
        '\n\n' +
        msg,
    )
  ) {
...
window.testTimeTheme = function (mockHour) {
  if (currentUser && currentUser.theme_preference === 'time') {
    ...
    console.log(
      `Mocking time as ${mockHour}:00 - Theme is now ${isLight ? 'light' : 'dark'}`,
    );
  } else {
    console.log(
      "Please set your theme to 'Time of Day' in Account settings first.",
    );
  }
};
...
document
  ...
  .addEventListener('submit', async (e) => {
    ...
    const payload = {
      ...
      notification_prefs: document.getElementById('acc-notifications').checked
        ? 'enabled'
        : 'disabled',
...
function showTab(tabName, skipHistory = false) {
  ...
  if (
    ADMIN_ONLY_TABS.includes(tabName) &&
    !(
      currentUser &&
      (currentUser.role === 'admin' || currentUser.role === 'owner')
    )
...
async function loadAnalytics() {
  ...
  if (res.ok) {
      ...
      charts['myBandwidth'] = new Chart(ctx, {
        ...
        data: {
          ...
          datasets: [
            {
              label: 'Data In',
              data: data.personal.daily.map((d) => d.bytes_in),
              borderColor: '#3b82f6',
            ...
            {
              label: 'Data Out',
              data: data.personal.daily.map((d) => d.bytes_out),
              borderColor: '#10b981',
    ...
    if (
      data.personal &&
      data.personal.tunnels &&
      data.personal.tunnels.length > 0
    ) {
    ...
    if (data.global) {
      ...
      if (data.global.daily) {
        const ctx = document
          .getElementById('globalBandwidthChart')
          .getContext('2d');
        ...
        charts['globalBandwidth'] = new Chart(ctx, {
          ...
          data: {
            ...
            datasets: [
              {
                label: 'Total Data In',
                data: data.global.daily.map((d) => d.bytes_in),
                borderColor: '#6366f1',
              ...
              },
              {
                ...
                data: data.global.daily.map((d) => d.bytes_out),
                borderColor: '#f43f5e',
        ...
        charts['topUsers'] = new Chart(ctx, {
          ...
          data: {
            labels: data.global.top_users.map(
              (u) => (u.email || 'Anonymous').split('@')[0],
            ),
            datasets: [
              {
                label: 'Total Bandwidth',
                data: data.global.top_users.map(
                  (u) => u.bytes_in + u.bytes_out,
                ),
...
async function loadTokens() {
  ...
  if (res.ok) {
    ...
    renderTable('tokens-table-body', tokens, (t) => {
      const isRevoked =
        t.revoked_at != null && !t.revoked_at.startsWith('0001-01-01');
      const isExpired =
        t.expires_at &&
  ...
  .addEventListener('submit', async (e) => {
    ...
    try {
      ...
      if (res.ok) {
        msg.textContent =
          data.message ||
          'Registration request submitted. Please check your email.';
...
document.getElementById('email-form').addEventListener('submit', async (e) => {
  ...
  try {
    const res = await fetch(
      '/api/auth/magic-link?lang=' + encodeURIComponent(currentLanguage),
      {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
...
async function loadUsers() {
  ...
  if (res.ok) {
    ...
    renderTable('users-table-body', users, (u) => {
      ...
      const dotShadow = u.portal_active
        ? 'box-shadow: 0 0 8px #10b981; text-shadow: 0 0 4px #10b981;'
        : '';
      ...
      const tunnelsBadge =
        tunnelsCount > 0
          ? `<span class="badge" onclick="openUserDetailsModal('${userJsonStr}')" style="cursor: pointer; background: rgba(99,102,241,0.15); color: #818cf8; border: 1px solid rgba(99,102,241,0.3); padding: 2px 6px; font-size: 11px; margin-left: 8px;" title="Click to view tunnels">🔌 ${tunnelsCount} Tunnel${tunnelsCount > 1 ? 's' : ''}</span>`
          : '';
      ...
      const lastSeenText = u.portal_active
        ? '<span style="color: #10b981; font-weight: 500;">Active Now</span>'
        : renderTimestamp(u.last_login_at);
...
async function deleteBlacklist(ip) {
  await fetch(`/api/admin/blacklist/${encodeURIComponent(ip)}`, {
    method: 'DELETE',
  });
  loadBlacklist();
}
...
function renderTimestamp(utcDateStr) {
  ...
  const hasTimezone =
    /Z$/i.test(utcDateStr) || /[+-]\d{2}(:?\d{2})?$/.test(utcDateStr);
  const parseStr = hasTimezone
    ? utcDateStr
    : utcDateStr.includes(' ')
...
function formatTimeRemaining(expiryDateStr, isRevoked) {
  ...
  const hasTimezone =
    /Z$/i.test(expiryDateStr) || /[+-]\d{2}(:?\d{2})?$/.test(expiryDateStr);
  const parseStr = hasTimezone
    ? expiryDateStr
    : expiryDateStr.includes(' ')
      ? expiryDateStr.replace(' ', 'T') + 'Z'
      : expiryDateStr + 'Z';
...
function promptTargetedMessage(uid, email) {
  ...
  document.getElementById('targeted-message-desc').innerText =
    'Send a direct toast alert to ' + email;
...
async function sendTargetedMessage() {
  ...
  if (!msg)
    return showToast(t('toast_enter_message', 'Please enter a message.'));
...
async function startMFASetup() {
  try {
    ...
    if (res.ok) {
      ...
      document.getElementById('mfa-qr-display').src =
        `https://api.qrserver.com/v1/create-qr-code/?size=180x180&data=${encodeURIComponent(data.otpauth_url)}`;
      document.getElementById('mfa-verify-code').value = '';
      document.getElementById('mfa-modal').style.display = 'flex';
    } else {
      showToast(
        t('mfa_toast_setup_fetch_failed', 'Failed to fetch MFA setup details.'),
        'danger',
      );
    }
  } catch (err) {
    showToast(
      t('mfa_toast_setup_network_error', 'Network error initiating MFA setup.'),
      'danger',
    );
  }
}
...
async function confirmEnableMFA() {
  ...
  if (code.length !== 6) {
    return showToast(
      t('mfa_toast_enter_code', 'Please enter a 6-digit code.'),
      'warning',
    );
  }
  ...
  try {
    ...
    if (res.ok) {
      ...
      closeMFAModal();
      showToast(
        t('mfa_toast_enabled_success', 'MFA enabled successfully!'),
        'success',
      );
    } else {
      const err = await res.json();
      showToast(
        err.error || t('mfa_toast_verification_failed', 'Verification failed.'),
        'danger',
      );
    }
  } catch (err) {
    showToast(
      t('mfa_toast_setup_network_error', 'Network error completing setup.'),
      'danger',
    );
  }
}
...
async function disableMFA() {
  ...
  if (code.length !== 6) {
    return showToast(
      t(
        'mfa_toast_enter_deactivate_code',
  ...
  try {
    ...
    if (res.ok) {
      ...
      renderMFAPanel();
      showToast(
        t('mfa_toast_disabled_success', 'MFA disabled successfully.'),
        'success',
      );
    } else {
      const err = await res.json();
      showToast(
        err.error || t('mfa_toast_disabled_failed', 'Failed to disable MFA.'),
        'danger',
      );
    }
  } catch (err) {
    showToast(
      t(
        'mfa_toast_deactivate_network_error',
        'Network error deactivating MFA.',
      ),
      'danger',
    );
  }
...
async function adminResetMFA(id) {
  if (
    !confirm(
      'Are you sure you want to disable Multi-Factor Authentication for this user? They will only need their magic link to log in.',
    )
  )
    return;
...
function updateMaintenanceModeUI(active, hardActive) {
  ...
  if (hardStatusText && hardToggleBtn) {
    if (hardActive) {
      hardStatusText.innerHTML =
        window.translations && window.translations['maint_iron_active']
          ? window.translations['maint_iron_active']
          : `Status: <span style=\"color: #ef4444; font-weight: 600;\">ACTIVE 🔴</span>`;
      hardToggleBtn.innerHTML =
        window.translations && window.translations['maint_btn_disable_hard']
          ? window.translations['maint_btn_disable_hard']
      ...
      hardStatusText.innerHTML =
        window.translations && window.translations['maint_iron_inactive']
          ? window.translations['maint_iron_inactive']
          : `Status: <span style=\"color: var(--text-muted);\">INACTIVE 🟢</span>`;
      hardToggleBtn.innerHTML =
        window.translations && window.translations['maint_btn_enable_hard']
          ? window.translations['maint_btn_enable_hard']
          : `Enable Iron Curtain`;
...
async function toggleSoftMaintenanceMode() {
  let nextState = true;
  if (
    globalMaintenanceActive === 'true' ||
    globalMaintenanceActive === 'pending'
  ) {
    nextState = false;
  }
  ...
  const durationVal =
    parseInt(document.getElementById('maint-soft-duration').value) || 30;
...
async function toggleHardMaintenanceMode() {
  ...
  if (nextState) {
    ...
    const durationVal =
      parseInt(document.getElementById('maint-hard-duration').value) || 60;
    ...
    if (secondConfirm !== 'LOCKOUT') {
      showToast(
        'Lockdown cancelled: confirmation word did not match.',
        'warning',
      );
      return;
    }
    ...
    try {
      ...
      if (res.ok) {
        ...
        updateMaintenanceModeUI(data.maintenance_mode, data.iron_curtain);
        showToast(
          'Nginx Iron Curtain activated. You will be disconnected shortly.',
          'danger',
        );
        setTimeout(() => {
          window.location.reload();
...
function openDeleteAccountModal() {
  ...
  document.getElementById('delete-acc-email-hint').innerText =
    currentUser.email;
...
async function submitSelfDeleteAccount() {
  const inputVal = document
    .getElementById('delete-acc-confirm-input')
    .value.trim();
  if (!inputVal)
    return showToast('Please type your email to confirm.', 'danger');

  if (inputVal.toLowerCase() !== currentUser.email.toLowerCase()) {
    return showToast(
      'Entered email address does not match your account email.',
      'danger',
    );
  }
  ...
  try {
    ...
    if (res.ok) {
      alert(
        'Your account has been permanently deleted and anonymised in accordance with your Right to Be Forgotten. You will now be redirected.',
      );
      window.location.reload();
    } else {
      const err = await res.json();
...
async function adminDeleteUser(email) {
  ...
  if (confirmation !== 'DELETE') {
    return showToast(
      'Account deletion cancelled (incorrect confirmation string).',
      'warning',
    );
  }
  ...
  try {
    ...
    if (res.ok) {
      showToast(
        `User ${email} has been permanently deleted and anonymised.`,
        'success',
      );
      loadUsers();
    } else {
...
async function submitUserQuota() {
  const limit =
    parseInt(document.getElementById('user-quota-input').value) || 0;
  const res = await fetch(
    '/api/admin/users/' + encodeURIComponent(activeQuotaEmail),
    {
      method: 'PATCH',
      headers: { 'Content-Type': 'application/json' },
...
async function submitTunnelOverride() {
  const limit =
    parseInt(document.getElementById('tunnel-override-input').value) || 0;
  ...
  if (res.ok) {
    ...
    const err = await res.json();
    showToast(
      'Failed to override limit: ' + (err.error || 'Unknown error'),
      'danger',
    );
  }
}
async function updateUserPreferredDomain() {
  ...
  try {
    const res = await fetch(
      `/api/admin/users/${_currentModalUserId}/preferred-domain`,
      {
        method: 'PUT',
        headers: {
    ...
    if (res.ok) {
      ...
      const err = await res.json();
      showToast(
        'Failed to update preferred domain: ' + (err.error || 'Unknown error'),
        'danger',
      );
    }
  } catch (e) {
...
async function openUserDetailsModal(userJsonEncoded) {
  ...
  document.getElementById('detail-user-name').innerText =
  ...
  if (m === 'invite') {
    originHTML =
      '<span class="badge" style="background: rgba(99,102,241,0.15); color: #818cf8; border: 1px solid rgba(99,102,241,0.3);">✉ Invite</span>';
  } else if (m === 'registration') {
    originHTML =
      '<span class="badge" style="background: rgba(16,185,129,0.15); color: #34d399; border: 1px solid rgba(16,185,129,0.3);">📝 Registration</span>';
  } else if (m.startsWith('sso - liferay') || m === 'liferay') {
    originHTML =
      '<span class="badge" style="background: rgba(30,120,220,0.15); color: #60a5fa; border: 1px solid rgba(30,120,220,0.3);">🔑 SSO · Liferay</span>';
  } else if (m.startsWith('sso - keycloak') || m === 'keycloak') {
    originHTML =
      '<span class="badge" style="background: rgba(239,68,68,0.15); color: #f87171; border: 1px solid rgba(239,68,68,0.3);">🔑 SSO · Keycloak</span>';
  } else {
  ...
  // Joined Date
  document.getElementById('detail-user-joined').innerHTML = renderTimestamp(
    u.created_at,
  );

  // API Quota
  document.getElementById('detail-user-quota').innerText = u.rate_limit
    ? `${u.rate_limit} RPS`
    : 'Unlimited';
  ...
  if (obStatus === 'completed') {
    ...
    obBadgeHTML =
      '<span class="badge" style="background: rgba(59, 130, 246, 0.15); color: #60a5fa; border: 1px solid rgba(59,130,246,0.3);">In Progress</span>';
  ...
  document.getElementById('detail-user-onboarding-status').innerHTML =
    obBadgeHTML;

  const lastStep = u.onboarding_last_step || 'N/A';
  const reruns = u.onboarding_reruns || 0;
  document.getElementById('detail-user-onboarding-detail').innerText =
    `Last Step: ${lastStep} | Reruns: ${reruns}`;

  // Connected Tunnels Table
  const tunnels = u.active_tunnels || [];
  document.getElementById('detail-user-tunnels-count').innerText =
    tunnels.length;
...
function showInstallerGuideModal() {
  ...
  document.getElementById('guide-script-macos').innerText =
    `curl -fsSL ${window.location.origin}/install | sh`;
  document.getElementById('guide-script-windows').innerText =
    `irm ${window.location.origin}/install.ps1 | iex`;
  document.getElementById('guide-cmd-linux').innerText =
    `curl -fsSL ${window.location.origin}/install | sh`;

  // If downloads are disabled, hide the direct download buttons in the guide modal
  const isDlDisabled =
    window.latestVersionData &&
    window.latestVersionData.disable_client_downloads;
  document.getElementById('guide-dl-macos').style.display = isDlDisabled
    ? 'none'
    : 'flex';
  document.getElementById('guide-dl-windows').style.display = isDlDisabled
    ? 'none'
    : 'block';
  document.getElementById('guide-dl-linux').style.display = isDlDisabled
    ? 'none'
    : 'flex';

  // If Scoop or Brew are disabled, hide them
  const isBrewDisabled =
    window.latestVersionData && window.latestVersionData.disable_brew;
  const isScoopDisabled =
    window.latestVersionData && window.latestVersionData.disable_scoop;
  document.getElementById('guide-macos-brew-section').style.display =
    isBrewDisabled ? 'none' : 'block';
  document.getElementById('guide-windows-scoop-section').style.display =
    isScoopDisabled ? 'none' : 'block';

  // Dynamically update Direct script installer labels if Homebrew/Scoop are disabled
  const macosDirectLabel = document.querySelector(
    '[data-i18n="guide_macos_direct"]',
  );
  if (macosDirectLabel) {
    if (isBrewDisabled) {
      macosDirectLabel.innerHTML =
        window.translations &&
        window.translations['guide_macos_direct_recommended']
          ? window.translations['guide_macos_direct_recommended']
          : 'Recommended via Shell Script:';
    } else {
      macosDirectLabel.innerHTML =
        window.translations && window.translations['guide_macos_direct']
          ? window.translations['guide_macos_direct']
          : 'Direct Installation Script (Alternative):';
    }
  }
  const windowsDirectLabel = document.querySelector(
    '[data-i18n="guide_windows_direct"]',
  );
  if (windowsDirectLabel) {
    if (isScoopDisabled) {
      windowsDirectLabel.innerHTML =
        window.translations &&
        window.translations['guide_windows_direct_recommended']
          ? window.translations['guide_windows_direct_recommended']
          : 'Recommended via PowerShell Script:';
    } else {
      windowsDirectLabel.innerHTML =
        window.translations && window.translations['guide_windows_direct']
          ? window.translations['guide_windows_direct']
...
async function kickTunnelFromUserModal(subdomain, userJsonEncoded) {
  ...
  if (
    ...
    if (res.ok) {
      ...
      const err = await res.json();
      showToast(
        'Failed to kick tunnel: ' + (err.error || 'Unknown error'),
        'danger',
      );
    }
  }
...
function populateTunnelDetails(t) {
  ...
  const visitorTbody = document.getElementById(
    'detail-tunnel-visitor-ips-tbody',
  );

  // Populate headers
  const headersContainer = document.getElementById(
    'detail-tunnel-headers-container',
  );
  if (headersContainer) {
  ...
  const ownerContainer = document.getElementById(
    'detail-tunnel-owner-container',
  );
...
async function addTunnelHeader() {
  if (!activeDetailTunnelSubdomain) return;
  const t = (currentUser.tunnels || []).find(
    (x) => x.subdomain_prefix === activeDetailTunnelSubdomain,
  );
  if (!t) return;
...
async function updateTunnelHeadersAPI(tunnel, headers) {
  try {
    ...
    if (res.ok) {
      ...
      const err = await res.json();
      showToast(
        'Failed to update headers: ' + (err.error || 'Unknown error'),
        'danger',
      );
    }
  } catch (e) {
  ...
  try {
    ...
    if (res.ok) {
      ...
      if (Array.isArray(data)) {
        ...
        document.getElementById('whats-new-title').innerText =
          `What's New in ${escapeHTML(data.version || 'this version')}`;
        const ul = document.createElement('ul');
        ul.style.paddingLeft = '20px';
...
async function loadReservations() {
  ...
  try {
    ...
    if (res.ok) {
      ...
      if (limit < 0) {
        document.getElementById('reservation-quota-text').innerText =
          `${used} / ∞ subdomains reserved`;
        document.getElementById('reservation-quota-bar').style.width = `0%`;
      } else {
        document.getElementById('reservation-quota-text').innerText =
          `${used} / ${limit} subdomains reserved`;
        const percent = limit > 0 ? (used / limit) * 100 : 0;
        document.getElementById('reservation-quota-bar').style.width =
      ...
      renderTable('reservations-table-body', list, (item) => {
        ...
        if (item.expires_at) {
          ...
          if (expiryDate < new Date()) {
            statusText =
              '<span class="badge danger" style="background: rgba(239, 68, 68, 0.15); color: #f87171; border: 1px solid rgba(239, 68, 68, 0.3);">Quarantined</span>';
          } else if (item.extension_requested) {
            statusText =
              '<span class="badge warning" style="background: rgba(245, 158, 11, 0.15); color: #fbbf24; border: 1px solid rgba(245, 158, 11, 0.3);">Extension Requested</span>';
          }
        }
...
async function generateRandomSubdomain() {
  ...
  try {
    const res = await fetch(
      `/api/portal/generate-subdomain?style=${encodeURIComponent(style)}`,
    );
    if (res.ok) {
      const data = await res.json();
...
async function reserveSubdomain() {
  const sub = document
    .getElementById('res-subdomain')
    .value.trim()
    .toLowerCase();
  const dom = document.getElementById('res-domain').value;
  if (!sub)
    return showToast('Please enter or generate a subdomain prefix', 'danger');
  ...
  try {
    ...
    if (res.ok) {
      ...
      const err = await res.json();
      showToast(
        'Failed to reserve: ' + (err.error || 'Unknown error'),
        'danger',
      );
    }
  } catch (e) {
...
async function requestExtension(id) {
  try {
    ...
    if (res.ok) {
      ...
      const err = await res.json();
      showToast(
        'Failed to request extension: ' + (err.error || 'Unknown error'),
        'danger',
      );
    }
  } catch (e) {
  ...
  if (
    ...
    try {
      ...
      if (res.ok) {
        ...
        const err = await res.json();
        showToast(
          'Failed to release: ' + (err.error || 'Unknown error'),
          'danger',
        );
      }
    } catch (e) {
...
async function approveExtension(id, days, permanent) {
  try {
    const res = await fetch(
      `/api/admin/reservations/${encodeURIComponent(id)}/approve-extension`,
      {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
    ...
    if (res.ok) {
      ...
      const err = await res.json();
      showToast(
        'Failed to approve extension: ' + (err.error || 'Unknown error'),
        'danger',
      );
    }
  } catch (e) {
...
async function demoteReservation(id) {
  try {
    const res = await fetch(
      `/api/admin/reservations/${encodeURIComponent(id)}/demote`,
      {
        method: 'POST',
      },
    );
    ...
    if (res.ok) {
      ...
      const err = await res.json();
      showToast(
        'Failed to demote: ' + (err.error || 'Unknown error'),
        'danger',
      );
    }
  } catch (e) {
...
async function submitUserResLimit() {
  ...
  try {
    const res = await fetch(
      `/api/admin/users/${encodeURIComponent(activeResLimitEmail)}/limit`,
      {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
    ...
    if (res.ok) {
      ...
      const err = await res.json();
      showToast(
        'Failed to update limit: ' + (err.error || 'Unknown error'),
        'danger',
      );
    }
  } catch (e) {
...
function openUserTunnelsLimitModal(email, currentLimit) {
  ...
  document.getElementById('user-tunnels-limit-input').value =
    currentLimit || '';
...
async function submitUserTunnelsLimit() {
  ...
  try {
    const res = await fetch(
      `/api/admin/users/${encodeURIComponent(activeTunnelsLimitEmail)}/tunnels-limit`,
      {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
...
async function extendToken(id, days) {
  try {
    ...
    if (res.ok) {
      ...
      const err = await res.json();
      showToast(
        'Failed to extend token: ' + (err.error || 'Unknown error'),
        'danger',
      );
    }
  } catch (e) {
...
window.toggleActionMenu = function (menuId, event) {
  ...
  if (menu && show && event) {
    ...
    const btn =
      event.currentTarget ||
      (event.target && event.target.closest('.action-menu-btn')) ||
      event.target;
...
window.addEventListener('touchstart', handleModalOverlayClick, {
  passive: true,
});
// ----------------------------------------------------
// NETWORK HEALTH
...
async function loadNetworkHealth() {
  ...
  try {
    ...
    if (selectAllTh)
      selectAllTh.classList.toggle('hidden', !edgePowerActionsEnabled);
    ...
    if (!outboundOk) {
      if (!banner) {
        ...
        banner.style =
          'padding: 16px; background: rgba(239, 68, 68, 0.1); border-left: 4px solid var(--danger); color: var(--danger); border-radius: 8px; margin-bottom: 24px; font-size: 13px; font-weight: 500; display: flex; align-items: center; gap: 8px;';
        banner.innerHTML =
    ...
    if (keys.length === 0) {
      ...
      keys.sort().forEach((id) => {
        ...
        let timeSince = h.last_check_at
          ? Math.max(0, Math.floor(Date.now() / 1000 - h.last_check_at)) +
            's ago'
          : 'Never';
        let errMsg = h.error_message
          ? `<span style="color:var(--danger); font-size:12px;">${escapeHTML(h.error_message)}</span>`
          : '';
        let localTimeText = formatNodeLocalTime(h.timezone);
        let uptimeText =
          isOnline && h.online_since
            ? formatUptime(
                Math.max(0, Math.floor(Date.now() / 1000) - h.online_since),
              )
            : '-';
...
function initSidebarSections() {
  ['personal', 'administration', 'reporting', 'maintenance'].forEach(
    (sectionId) => {
      const content = document.getElementById('section-' + sectionId);
      if (!content) return;
      const header = content.previousElementSibling;
      const collapsedVal = localStorage.getItem(
        'sidebar_collapsed_' + sectionId,
      );
...
window.kickEdgeTunnels = function (nodeId) {
  if (
    confirm(
      'Are you sure you want to kick ALL active tunnels on edge node ' +
        nodeId +
        '?',
    )
  ) {
...
window.disableEdgeMaintenance = function (nodeId) {
  if (
    confirm('Are you sure you want to disable maintenance on ' + nodeId + '?')
  ) {
...
window.startEdgeNode = function (nodeId) {
  if (confirm(`Start the ${nodeId} instance?`))
    triggerEdgePowerAction(nodeId, 'start');
};
window.stopEdgeNode = function (nodeId) {
  if (
    ...
    triggerEdgePowerAction(nodeId, 'stop');
};
window.restartEdgeInstance = function (nodeId) {
  if (
    ...
    triggerEdgePowerAction(nodeId, 'restart');
};

window.toggleEdgeSelection = function (nodeId, checked) {
  if (checked) edgeSelectedIds.add(nodeId);
  else edgeSelectedIds.delete(nodeId);
  updateEdgeBulkToolbar();
};
...
window.toggleAllEdgeSelection = function (checked) {
  document.querySelectorAll('.edge-select-checkbox').forEach((cb) => {
    cb.checked = checked;
    if (checked) edgeSelectedIds.add(cb.dataset.nodeId);
    else edgeSelectedIds.delete(cb.dataset.nodeId);
  });
  updateEdgeBulkToolbar();
};
...
window.bulkEdgeAction = async function (action) {
  ...
  if (
    !confirm(
      `${action.charAt(0).toUpperCase() + action.slice(1)} ${nodeIds.length} selected node(s)?`,
    )
  )
    return;
  ...
  try {
    ...
    if (res.ok) {
      ...
      populateTimezoneSelect(
        document.getElementById('edge-schedule-timezone'),
        sched.timezone || '',
      );
      document.getElementById('edge-schedule-enabled').checked =
        sched.enabled !== false;
...
window.submitEdgeSchedule = async function () {
  ...
  const timezone = document
    .getElementById('edge-schedule-timezone')
    .value.trim();
  ...
  try {
    const res = await fetch(
      `/api/admin/edge/${encodeURIComponent(edgeScheduleNodeId)}/schedule`,
      {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
  ...
  if (type === 'mfa_setup') {
    try {
      ...
      if (res.ok) {
        ...
        document.getElementById('intercept-mfa-secret-display').innerText =
          data.secret;
        document.getElementById('intercept-mfa-qr-display').src =
          `https://api.qrserver.com/v1/create-qr-code/?size=180x180&data=${encodeURIComponent(data.otpauth_url)}`;
        document.getElementById('intercept-mfa-verify-code').value = '';
      } else {
        showToast(
          t(
            'mfa_toast_setup_fetch_failed',
            'Failed to fetch MFA setup details.',
          ),
          'danger',
        );
      }
      ...
      showToast(
        t(
          'mfa_toast_setup_network_error',
          'Network error initiating MFA setup.',
        ),
        'danger',
      );
    }
...
window.confirmInterceptMFA = async function () {
  const code = document
    .getElementById('intercept-mfa-verify-code')
    .value.trim();
  if (code.length !== 6) {
    return showToast(
      t('mfa_toast_enter_code', 'Please enter a 6-digit code.'),
  ...
  try {
    ...
    showToast(
      t('mfa_toast_verify_network_error', 'Network error verifying MFA.'),
      'danger',
    );
  }
};
  ...
  const driverObj = window.driver.js.driver({
    ...
    steps: [
      {
        ...
        popover: {
          ...
          description: t(
            'tour_welcome_desc',
            "Let's take a quick 1-minute interactive tour of your developer dashboard to show you how to manage your subdomains and tunnels.",
          ),
          side: 'over',
          align: 'center',
        },
      },
      {
        ...
        popover: {
          ...
          description: t(
            'tour_sidebar_desc',
            'Use the sidebar to jump between the Overview, API Tokens, Subdomain Reservations, Active Tunnels, and Analytics tabs.',
          ),
          side: 'right',
          align: 'start',
        },
      },
      {
        ...
        popover: {
          ...
          description: t(
            'tour_reservations_desc',
            'Here you can reserve wildcard subdomains on our public domains (e.g. lfr-demo.se), view expiration dates, and request lease extensions.',
          ),
          side: 'right',
          align: 'start',
        },
      ...
      },
      {
        ...
        popover: {
          ...
          description: t(
            'tour_tokens_desc',
            'Generate secure CLI tokens here. You will need to configure your local CLI client with one of these tokens to connect.',
          ),
          side: 'right',
          align: 'start',
        },
      ...
      },
      {
        ...
        popover: {
          ...
          description: t(
            'tour_tunnels_desc',
            'Inspect your currently connected client tunnels in real-time. View live traffic metrics, latency, and kick inactive leases if needed.',
          ),
          side: 'right',
          align: 'start',
        },
      ...
      },
      {
        ...
        popover: {
          ...
          description: t(
            'tour_analytics_desc',
            'Monitor your total bandwidth utilization (bytes uploaded/downloaded) over time in a beautiful responsive chart.',
          ),
          side: 'right',
          align: 'start',
        },
      ...
      },
      {
        ...
        popover: {
          ...
          description: t(
            'tour_settings_desc',
            'Update your preferred name, language, theme preference, and configure Multi-Factor Authentication (MFA) for added security.',
          ),
          side: 'right',
          align: 'start',
        },
      ...
      },
      {
        ...
        popover: {
          ...
          description: t(
            'tour_end_desc',
            'You are all set. Go ahead and start using Liferay Tunnel!',
          ),
          side: 'over',
          align: 'center',
        },
    ...
    onDestroyed: (el) => {
      ...
      const status =
        ...
        (stepIdx !== undefined && stepIdx >= totalSteps - 1)
          ? 'completed'
          : 'aborted';
      ...
      fetch('/api/me/onboarding', {
        ...
        body: JSON.stringify({
          status: status,
          last_step: stepIdx !== undefined ? 'step_' + stepIdx : 'unknown',
          is_rerun: false,
        }),
      }).catch((err) =>
        console.error(
          'Failed to report onboarding completion/abort telemetry',
          err,
        ),
      );
...
function renderConfigTree() {
  ...
  if (treeEl) {
    ...
    container.innerHTML =
      '<div style="color: var(--text-muted); text-align: center;">No matching configuration items found.</div>';
  }
}
...
function buildTreeHTML(val, filterText, path = '') {
  ...
  if (
    filterText &&
    !cleanPathLower.includes(filterText) &&
    !valLower.includes(filterText)
  ) {
    return null;
  }
    ...
    if (res.ok) {
      ...
      if (data.default_domain) {
        ...
        setSelectValueSafely(
          ...
          data.default_domain,
          (v) =>
            `${v} ${t('domain_not_supported', '(not in supported domains)')}`,
        );
      }

      // Populate and configure Vanity Domain Hook controls
      const isOwner = currentUser && currentUser.role === 'owner';
      document.getElementById('system-enable-vanity-hook').checked =
        !!data.enable_vanity_domain_hook;
      document.getElementById('system-enable-vanity-hook').disabled = !isOwner;
      document.getElementById('system-vanity-hook-path').value =
        data.vanity_domain_hook_path || '';
      document.getElementById(
        'system-vanity-hook-owner-warning',
...
async function saveSystemSettings() {
  ...
  const enableVanityHook = document.getElementById(
    'system-enable-vanity-hook',
  ).checked;
  const vanityHookPath = document.getElementById(
    'system-vanity-hook-path',
  ).value;
...
function renderViewAs() {
  ...
  if (viewAs) {
    ...
    label.textContent =
      'Previewing as ' + viewAs + ' \u2014 read-only, no changes can be made';
  ...
  select.onchange = () => {
    if (select.value) setViewAs(select.value);
  };
  bar.appendChild(label);
