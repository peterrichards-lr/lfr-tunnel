...
const TIMEZONE_GROUPS: [string, string[]][] = (() => {
  ...
  return Object.keys(groups)
    .sort()
    .map((region) => [region, groups[region].sort()]);
})();
...
export default function EdgeScheduleModal({
  nodeId,
  onClose,
  onSaved,
}: EdgeScheduleModalProps) {
  const { t } = useI18n();
    ...
    (async () => {
      try {
        ...
        if (e.response?.status !== 404) {
          showToast(
            e.response?.data?.error || 'Failed to load schedule.',
            'error',
          );
        }
      } finally {
  ...
  const handleSave = async () => {
    if (!stopTime || !startTime || !timezone.trim()) {
      showToast(
        'Stop time, start time, and timezone are all required.',
        'error',
      );
      return;
    }
