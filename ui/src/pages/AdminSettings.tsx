...
export default function AdminSettings() {
  ...
  const toggleSoftMaintenanceMode = async () => {
    ...
    if (
      !(await showConfirm(
        nextState ? 'Enable Soft Maintenance' : 'Disable Soft Maintenance',
        promptMsg,
      ))
    )
      return;
    ...
    try {
      ...
      showToast(
        e.response?.data?.error || 'Failed to update maintenance mode',
        'error',
      );
    }
  };
  ...
  const toggleHardMaintenanceMode = async () => {
    ...
    if (nextState) {
      ...
      try {
        ...
        showToast(
          e.response?.data?.error || 'Failed to activate Iron Curtain',
          'error',
        );
      }
    } else {
      ...
      try {
        ...
        showToast(
          e.response?.data?.error || 'Failed to disable Iron Curtain',
          'error',
        );
      }
    }
  ...
  const triggerBackup = async () => {
    try {
      ...
      showToast(
        e.response?.data?.error || 'Failed to trigger backup.',
        'error',
      );
    } finally {
    ...
    <div>
      ...
      <div className="card mb-xl">
        ...
        <p className="text-muted text-sm mb-xl">
          Configure maintenance gates to manage system upgrades and deployments.
          Soft maintenance gracefully alerts and migrates standard sessions,
          while the Iron Curtain locks down the VPS web proxy completely.
        </p>
        ...
        <div className="col-2">
          ...
          <div className="p-lg border rounded flex flex-col gap-md">
            <div className="flex justify-between items-center">
              ...
              <span
                className={`badge ${maintenance.status === 'true' ? 'danger' : maintenance.status === 'pending' ? 'warning' : 'success'}`}
              >
            ...
            {maintenance.status !== 'false' ? (
              <div className="p-md rounded-sm text-sm flex flex-col gap-xs surface-subtle">
                <div>
                  <strong>Action:</strong> {maintenance.action}
                </div>
                <div>
                  <strong>Reason:</strong> {maintenance.reason}
                </div>
                <div>
                  <strong>Scheduled/Started:</strong>{' '}
                  {formatDate(maintenance.start_time)}
                </div>
                {maintenance.duration > 0 && (
                  <div>
                    <strong>Duration:</strong> {maintenance.duration} minutes
                  </div>
                )}
              </div>
            ) : (
              <>
                <div className="form-group m-0">
                  <label className="form-label text-xs" htmlFor="field-4">
                    Action Name
                  </label>
                  <input
                    id="field-4"
                ...
                <div className="form-group m-0">
                  <label className="form-label text-xs" htmlFor="field-5">
                    Reason
                  </label>
                  <input
                    id="field-5"
                ...
                <div className="flex gap-md">
                  <div className="form-group m-0 flex-1">
                    <label className="form-label text-xs" htmlFor="field-6">
                      Duration (min)
                    </label>
                    <input
                      id="field-6"
                      ...
                      value={softDuration}
                      onChange={(e) =>
                        setSoftDuration(parseInt(e.target.value) || 0)
                      }
                    />
                  </div>
                  <div className="form-group m-0 flex-1">
                    <label className="form-label text-xs" htmlFor="field-7">
                      Countdown (min)
                    </label>
                    <select
                      id="field-7"
                      ...
                      value={softCountdown}
                      onChange={(e) =>
                        setSoftCountdown(parseInt(e.target.value) || 0)
                      }
                    >
          ...
          <div className="p-lg border rounded flex flex-col gap-md">
            <div className="flex justify-between items-center">
              <h5 className="m-0 text-md fw-semibold">
                Iron Curtain (Hard Lockdown)
              </h5>
              <span
                className={`badge ${maintenance.iron_curtain ? 'danger' : 'success'}`}
              >
            ...
            {!maintenance.iron_curtain ? (
              <>
                <div className="form-group m-0">
                  <label className="form-label text-xs" htmlFor="field-8">
                    Lockout Action
                  </label>
                  <input
                    id="field-8"
                ...
                <div className="form-group m-0">
                  <label className="form-label text-xs" htmlFor="field-9">
                    Lockout Reason
                  </label>
                  <input
                    id="field-9"
                ...
                <div className="form-group m-0">
                  <label className="form-label text-xs" htmlFor="field-10">
                    Duration (min)
                  </label>
                  <input
                    id="field-10"
                    ...
                    value={hardDuration}
                    onChange={(e) =>
                      setHardDuration(parseInt(e.target.value) || 0)
                    }
                  />
                </div>
              </>
              ...
              <div className="alert-banner alert-banner--danger flex-col items-start gap-xs">
                ...
                <div>
                  <strong>Expires in:</strong> {maintenance.duration} minutes
                </div>
              </div>
            )}
        ...
        <div className="card mb-xl">
          <div className="flex justify-between items-center mb-lg">
            <div>
              ...
              <div className="text-sm text-muted mt-xs">
                Manage and download automated database snapshots.
              </div>
            </div>
            <button
              className="btn btn-primary"
                ...
                {backups.length === 0 ? (
                  <tr>
                    <td colSpan={4} className="td-empty opacity-60">
                      No backups found yet. The first backup runs on server
                      startup.
                    </td>
                  </tr>
                ) : (
                  sortedBackups
                    ...
                    .map((b) => (
                      <tr key={b.filename} className="border-b">
                        <td className="td-cell--mono fw-medium">
                          {b.filename}
                        </td>
                        <td className="td-cell">
                          {formatSizeKB(b.size_bytes)}
                        </td>
            ...
            {sortedBackups.length > 0 && (
              <div className="pagination-row p-lg border-t">
                <div className="pagination-count">
                  ...
                  {Math.min((page + 1) * ROWS_PER_PAGE, sortedBackups.length)}{' '}
                  of {sortedBackups.length}
                </div>
                <div className="pagination-controls">
                  ...
                  <span className="pagination-page-label">
                    Page {page + 1} of{' '}
                    {Math.ceil(sortedBackups.length / ROWS_PER_PAGE)}
                  </span>
                  <button
                    className="btn btn-secondary py-xs px-md text-xs w-auto"
                    disabled={
                      (page + 1) * ROWS_PER_PAGE >= sortedBackups.length
                    }
                    onClick={() => setPage(page + 1)}
                  >
                    Next
                  ...
                  <button
                    ...
                    onClick={() =>
                      setPage(
                        Math.max(
                          0,
                          Math.ceil(sortedBackups.length / ROWS_PER_PAGE) - 1,
                        ),
                      )
                    }
                    disabled={
