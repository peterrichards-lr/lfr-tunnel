...
export default function Login() {
  ...
  useEffect(() => {
    ...
    if (magicToken) {
      ...
      axios
        .post('/api/auth/verify', { token: magicToken, lang: langParam })
        .then((res) => {
        ...
        .catch((err) => {
          setStatusMsg({
            text:
              ...
              t('error_magic_link_invalid', 'Invalid or expired magic link.'),
            isError: true,
          });
        });
    }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (mode === 'login') {
      try {
        ...
        setStatusMsg({
          text: t('magic_link_sent', 'Magic link sent! Check your email.'),
          isError: false,
        });
        setCountdown(60);
      } catch (err: any) {
        const errorText =
          err.response?.data?.error ||
          t('error_send_link', 'Failed to send link.');
      ...
      try {
        ...
        setStatusMsg({
          text: t(
            ...
            'Registration request sent! Please check your email.',
          ),
          isError: false,
        });
        ...
        const errorText =
          err.response?.data?.error ||
          t('error_register_request', 'Failed to submit registration request.');
        setStatusMsg({ text: errorText, isError: true });
      } finally {
        setIsSending(false);
      }
  ...
  const handleMfaSubmit = async (e: React.FormEvent) => {
    ...
    if (mfaCode.length !== 6) {
      setStatusMsg({
        text: t('error_mfa_length', 'Please enter a 6-digit code.'),
        isError: true,
      });
      return;
    }
    ...
    try {
      ...
      setStatusMsg({
        text:
          err.response?.data?.error ||
          t('error_mfa_invalid', 'Invalid verification code.'),
        isError: true,
      });
    } finally {
      setIsSending(false);
        ...
        {mfaRequired ? (
          <form id="mfa-form" onSubmit={handleMfaSubmit}>
            <h1 className="mt-0 text-center text-xl fw-bold tracking-tight">
              {t('mfa_verify_title', 'Multi-Factor Verification')}
            </h1>
            ...
            >
              {isSending
                ...
                : t('btn_verify_login', 'Verify & Login')}
            </button>
            {statusMsg.text && (
              <div
                className={`mt-lg text-center text-sm fw-medium ${statusMsg.isError ? 'text-danger' : 'text-success'}`}
              >
                {statusMsg.text}
              </div>
            )}
          ...
          <>
            ...
            <p className="text-center text-muted mb-xl text-base">
              {mode === 'login'
                ? t(
                    'portal_login_desc',
                    'Enter your email to request a secure passwordless login link.',
                  )
                : t(
                    'portal_register_desc',
              ...
              <div className="mt-xl text-center">
                ...
                >
                  {mode === 'login'
                    ...
                    : t('btn_login_with_email', 'Login with Email')}
                </button>
              </div>
            </form>
          </>
